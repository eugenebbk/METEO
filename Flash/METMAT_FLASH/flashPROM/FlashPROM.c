...
//extern uint8_t errorFLASHPROM;
extern statusSensor_t statusSensor;
...
//		HAL_CAN_AddTxMessage(&hcan, &canTxHeader, &canTxData[0], &TxMailBox);
	}
	else
	{
		#if DEBUG
...
//		if(HAL_FLASH_Program(WIDTHWRITE, res_addr, (uint32_t)*(buff+i)) != HAL_OK)
//		if(HAL_FLASH_Program(WIDTHWRITE, res_addr, *(buff+i)) != HAL_OK)
		{
//			uint32_t er = HAL_FLASH_GetError();
//			char str[64] = {0,};
...
//	HAL_UART_Transmit(&huart1, (uint8_t*)str, strlen(str), 100);
	...
	{
//		errorFLASHPROM = 1;
		statusSensor.bit.EEPROM_Error = 1;
		statusSensor.bit.CmdParamError = 1;
		#if DEBUG
