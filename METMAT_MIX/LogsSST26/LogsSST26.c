...
void SST26readAllLogs(uint16_t lastFreeID_log, uint8_t numbFlashMemory)
{
    ...
    uint32_t currentFreeAddrLog = sFLASH_SearchLastFreePageAdress(&lastFreeID_log, 4, numbFlashMemory);
...
// uint16_t searchLastLogID(uint8_t numbFlashMemory)
// {
//     uint32_t sFLASH_SearchLastFreePageAdress(uint16_t * ID_log, 4, numbFlashMemory)
// }