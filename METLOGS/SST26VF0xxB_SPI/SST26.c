...
//переделать поиск свободной ячейки лога
...
uint32_t sFLASH_SearchLastFreePageAdress(uint16_t *ID_log, uint8_t numbFlashMemory)
{
	...
	// for (id = 0; id < numbOfLogs; id++)
	{
		...
		sFLASH_ReadBuffer((uint8_t *)&idSearch, sFLASH_SPI_PAGE_SIZE * id, BYTE_CHECK_FREE, numbFlashMemory);
		/* Deselect the FLASH: Chip Select high */
		if (numbFlashMemory == 1)
		{
			sFLASH1_CS_HIGH();
